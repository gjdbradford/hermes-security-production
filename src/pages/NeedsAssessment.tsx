...
const NeedsAssessment = () => {
  ...
  const [additionalInfoText, _setAdditionalInfoText] = useState<string>('');
  const [selectedPentestType, setSelectedPentestType] = useState<string>('');
  const [selectedProductionEnvironment, setSelectedProductionEnvironment] = useState<boolean>(false);
  ...
  const [networkConcernsText, setNetworkConcernsText] = useState<string>('');
  
  // Infrastructure state
  const [selectedInfrastructureComponents, setSelectedInfrastructureComponents] = useState<string[]>([]);
  ...
  const [infrastructureConcernsText, setInfrastructureConcernsText] = useState<string>('');

  // CAPTCHA verification hook
  const { verifyCaptcha, isVerifying: isCaptchaVerifying } = useCaptchaVerification('needs_assessment_submit');
  ...
  const isStepValid = useCallback((step: number) => {
    ...
    const _emailValid = hasEmailFromUrl || (localEmail && !emailError && isValidEmail(localEmail));
    ...
    switch (step) {
      case 1:
        return !!selectedPentestType && (selectedProductionEnvironment || selectedStagingEnvironment) && !!selectedPreferredTime;
      case 2:
        return selectedServices.length > 0;
      case 3:
        return !!selectedWebCount && selectedWebTechnologies.length > 0 && selectedWebVulnerabilities.length > 0;
      case 4:
        return selectedMobilePlatforms.length > 0 && !!selectedMobileCount && selectedMobileFrameworks.length > 0 && selectedMobileFeatures.length > 0;
      case 5:
        return selectedApiTypes.length > 0 && !!selectedApiCount && selectedApiAuthMethods.length > 0 && selectedApiSensitiveData.length > 0;
      case 6:
        return selectedNetworkComponents.length > 0 && selectedNetworkProtocols.length > 0;
      case 7:
        return selectedInfrastructureComponents.length > 0 && selectedCloudPlatforms.length > 0 && selectedOperatingSystems.length > 0;
      case 8:
        return true;
      default:
        return true;
    }
  }, [selectedPentestType, selectedProductionEnvironment, selectedStagingEnvironment, selectedPreferredTime, selectedServices, selectedWebCount, selectedWebTechnologies, selectedWebVulnerabilities, selectedMobilePlatforms, selectedMobileCount, selectedMobileFrameworks, selectedMobileFeatures, selectedApiTypes, selectedApiCount, selectedApiAuthMethods, selectedApiSensitiveData, selectedNetworkComponents, selectedNetworkProtocols, selectedInfrastructureComponents, selectedCloudPlatforms, selectedOperatingSystems]);
  ...
  const BasicQuestionsStep = () => (
    <div className="space-y-8">
      ...
      <div className="space-y-6">
        <div>
          <label className="text-lg font-semibold block mb-3">* What type of pentest would you like to conduct?</label>
          <div className="grid gap-3">
            {[
              { value: 'white-box', label: 'White Box Pentest', description: 'Full knowledge of system architecture and code' },
              { value: 'gray-box', label: 'Gray Box Pentest', description: 'Partial knowledge of system architecture' },
              { value: 'black-box', label: 'Black Box Pentest', description: 'No prior knowledge of system architecture' },
        ...
        <div>
          <label className="text-lg font-semibold block mb-3">* Will this testing be done in a production environment?</label>
          <div className="space-y-3">
            <div className="flex items-center space-x-2">
              <input
                ...
                className="w-4 h-4"
              />
              <label htmlFor="productionEnvironment">Testing will be done in production environment</label>
            </div>
            <div className="flex items-center space-x-2">
              <input
                type="checkbox"
                ...
                className="w-4 h-4"
              />
              <label htmlFor="stagingEnvironment">Similar staging/development environment exists</label>
            </div>
          </div>
        </div>

        <div>
          <label className="text-lg font-semibold block mb-3">* At what time do you want these tests to be performed?</label>
          <select
        ...
        <div>
          <label className="text-lg font-semibold block mb-3">Does your company use any monitoring and/or intrusion protection/detection systems?</label>
          <div className="flex items-center space-x-2">
            <input
              type="checkbox"
  ...
  const ServiceSelectionStep = () => (
    <div className="space-y-8">
      <div className="text-center mb-6">
        ...
        <p className="text-muted-foreground">
          Select all areas that need to be tested
        </p>
      </div>

      <div className="grid gap-4">
        {[
          { id: 'website', label: 'Web Applications', description: 'Web applications, websites, and web services' },
          { id: 'mobile', label: 'Mobile Applications', description: 'iOS and Android mobile applications' },
          { id: 'api', label: 'API Endpoints', description: 'REST APIs, GraphQL, and web services' },
          { id: 'network', label: 'Network Infrastructure', description: 'Network devices, firewalls, and network security' },
          { id: 'infrastructure', label: 'Infrastructure', description: 'Servers, cloud infrastructure, and system security' }
        ].map(service => (
          <Card 
            key={service.id} 
            className={`cursor-pointer transition-all hover:shadow-md ${
              selectedServices.includes(service.id) ? 'ring-2 ring-primary bg-primary/5' : 'hover:bg-muted/50'
            }`}
  ...
  const WebQuestionsStep = () => (
    ...
    <div className="space-y-6">
        <div>
          <label className="text-lg font-semibold block mb-3">* How many web applications need to be tested?</label>
          <select
        ...
        <div>
          <label className="text-lg font-semibold block mb-3">* What technologies are used in your web applications?</label>
          <div className="grid grid-cols-2 gap-3">
            {[
              'React', 'Vue.js', 'Angular', 'PHP', 'Node.js', 'Python/Django', 
              'Ruby on Rails', 'ASP.NET', 'Java/Spring', 'WordPress', 'Drupal', 'Other'
            ].map(tech => (
              <div key={tech} className="flex items-center space-x-2">
                <input
                  ...
                  className="w-4 h-4"
                />
                <label htmlFor={`web-tech-${tech}`} className="text-sm">{tech}</label>
              </div>
            ))}
          </div>
        </div>
        ...
        <div>
          <label className="text-lg font-semibold block mb-3">* What types of vulnerabilities are you most concerned about?</label>
          <div className="space-y-3">
            {[
              ...
              <div key={vuln.value} className="flex items-center space-x-2">
                <input
                  ...
                  className="w-4 h-4"
                />
                <label htmlFor={`web-vuln-${vuln.value}`} className="text-sm">{vuln.label}</label>
              </div>
            ))}
          </div>
        </div>
        ...
        <div>
          <label className="text-lg font-semibold block mb-3">Do you have any specific areas of concern or known issues?</label>
          <textarea
            value=""
            onChange={(e) => {
              console.log('Web concerns:', e.target.value);
            }}
  ...
  const MobileQuestionsStep = () => (
    ...
    <div className="space-y-6">
        <div>
          <label className="text-lg font-semibold block mb-3">* What platforms do your mobile apps target?</label>
          <div className="grid grid-cols-2 gap-3">
            <div className="flex items-center space-x-2">
              <input
                type="checkbox"
                ...
                className="w-4 h-4"
              />
              <label htmlFor="mobile-ios" className="text-sm">iOS</label>
            </div>
            <div className="flex items-center space-x-2">
              <input
                type="checkbox"
                ...
                className="w-4 h-4"
              />
              <label htmlFor="mobile-android" className="text-sm">Android</label>
            </div>
          </div>
        </div>

        <div>
          <label className="text-lg font-semibold block mb-3">* How many mobile applications need testing?</label>
          <select
        ...
        <div>
          <label className="text-lg font-semibold block mb-3">* What development frameworks are used?</label>
          <div className="grid grid-cols-2 gap-3">
            {[
              ...
              <div key={framework} className="flex items-center space-x-2">
                <input
                  ...
                  className="w-4 h-4"
                />
                <label htmlFor={`mobile-framework-${framework}`} className="text-sm">{framework}</label>
              </div>
            ))}
          </div>
        </div>
        ...
        <div>
          <label className="text-lg font-semibold block mb-3">* Do your mobile apps use any of these features?</label>
          <div className="space-y-3">
            {[
              ...
              <div key={feature.value} className="flex items-center space-x-2">
                <input
                  ...
                  className="w-4 h-4"
                />
                <label htmlFor={`mobile-feature-${feature.value}`} className="text-sm">{feature.label}</label>
              </div>
            ))}
          </div>
        </div>
  ...
  const APIQuestionsStep = () => (
    <div className="space-y-8">
      <div className="text-center mb-6">
        ...
        <p className="text-muted-foreground">
          Tell us about your APIs that need security testing
        </p>
      </div>

    <div className="space-y-6">
        <div>
          <label className="text-lg font-semibold block mb-3">* What types of APIs do you have?</label>
          <div className="grid grid-cols-2 gap-3">
            {[
              ...
              <div key={apiType.value} className="flex items-center space-x-2">
                <input
                  ...
                  className="w-4 h-4"
                />
                <label htmlFor={`api-type-${apiType.value}`} className="text-sm">{apiType.label}</label>
              </div>
            ))}
          </div>
        </div>
        ...
        <div>
          <label className="text-lg font-semibold block mb-3">* How many API endpoints need testing?</label>
          <select
        ...
        <div>
          <label className="text-lg font-semibold block mb-3">* What authentication methods do your APIs use?</label>
          <div className="space-y-3">
            {[
              ...
              <div key={auth.value} className="flex items-center space-x-2">
                <input
                  ...
                  className="w-4 h-4"
                />
                <label htmlFor={`api-auth-${auth.value}`} className="text-sm">{auth.label}</label>
              </div>
            ))}
          </div>
        </div>
        ...
        <div>
          <label className="text-lg font-semibold block mb-3">* What sensitive data do your APIs handle?</label>
          <div className="space-y-3">
            {[
              ...
              <div key={data.value} className="flex items-center space-x-2">
                <input
                  ...
                  onChange={(e) => {
                    console.log('API sensitive data checkbox changed:', data.value, e.target.checked);
                    if (e.target.checked) {
                  ...
                  className="w-4 h-4"
                />
                <label htmlFor={`api-data-${data.value}`} className="text-sm">{data.label}</label>
              </div>
            ))}
          </div>
        </div>
  ...
  const NetworkStep = () => (
    <div className="space-y-8">
      <div className="text-center mb-6">
        ...
        <p className="text-muted-foreground">
          Tell us about your network that needs testing
        </p>
      </div>

    <div className="space-y-6">
        <div>
          <label className="text-lg font-semibold block mb-3">* What network components need testing?</label>
          <div className="grid grid-cols-2 gap-3">
            {[
              ...
              <div key={component.value} className="flex items-center space-x-2">
                <input
                  ...
                  className="w-4 h-4"
                />
                <label htmlFor={`network-${component.value}`} className="text-sm">{component.label}</label>
              </div>
            ))}
          </div>
        </div>
        ...
        <div>
          <label className="text-lg font-semibold block mb-3">* What network protocols are in use?</label>
          <div className="grid grid-cols-2 gap-3">
            {[
              'TCP/IP', 'HTTP/HTTPS', 'SSH', 'Telnet', 
              'SNMP', 'FTP/SFTP', 'DNS', 'DHCP', 'LDAP', 'Other'
            ].map(protocol => (
        ...
        <div>
          <label className="text-lg font-semibold block mb-3">Do you have any specific network security concerns?</label>
          <textarea
            value={networkConcernsText}
            onChange={(e) => {
              console.log('Network concerns:', e.target.value);
              setNetworkConcernsText(e.target.value);
            }}
  ...
  const InfrastructureStep = () => (
    ...
    <div className="space-y-6">
        <div>
          <label className="text-lg font-semibold block mb-3">* What infrastructure components need testing?</label>
          <div className="grid grid-cols-2 gap-3">
            {[
              ...
              <div key={component.value} className="flex items-center space-x-2">
                <input
                  ...
                  onChange={(e) => {
                    console.log('Infrastructure component selected:', component.value, e.target.checked);
                    if (e.target.checked) {
                      setSelectedInfrastructureComponents(prev => [...prev, component.value]);
                    } else {
                      setSelectedInfrastructureComponents(prev => prev.filter(c => c !== component.value));
                    }
                  }}
                  className="w-4 h-4"
                />
                <label htmlFor={`infra-${component.value}`} className="text-sm">{component.label}</label>
              </div>
            ))}
          </div>
        </div>
        ...
        <div>
          <label className="text-lg font-semibold block mb-3">* What cloud platforms do you use?</label>
          <div className="grid grid-cols-2 gap-3">
            {[
              ...
              <div key={cloud.value} className="flex items-center space-x-2">
                <input
                  ...
                  className="w-4 h-4"
                />
                <label htmlFor={`cloud-${cloud.value}`} className="text-sm">{cloud.label}</label>
              </div>
            ))}
          </div>
        </div>
        ...
        <div>
          <label className="text-lg font-semibold block mb-3">* What operating systems are in use?</label>
          <div className="grid grid-cols-2 gap-3">
            {[
              'Windows Server', 'Linux (Ubuntu)', 'Linux (CentOS/RHEL)', 'Linux (Debian)', 
              'macOS Server', 'FreeBSD', 'VMware ESXi', 'Other'
            ].map(os => (
        ...
        <div>
          <label className="text-lg font-semibold block mb-3">Do you have any specific infrastructure security concerns?</label>
          <textarea
            value={infrastructureConcernsText}
            onChange={(e) => {
              console.log('Infrastructure concerns:', e.target.value);
              setInfrastructureConcernsText(e.target.value);
            }}
    ...
    <div className="space-y-6">
      ...
      <div className="space-y-6">
        ...
        >
          ...
          <p><strong>Email:</strong> {hasEmailFromUrl ? emailFromUrl : localEmail}</p>
          <p><strong>Pentest Type:</strong> {selectedPentestType}</p>
            <p><strong>Preferred Time:</strong> {selectedPreferredTime}</p>
            <p><strong>Production Environment:</strong> {selectedProductionEnvironment ? 'Yes' : 'No'}</p>
            {additionalInfoText && (
        ...
        {selectedServices.includes('website') && (
          ...
          >
            ...
            <CardContent className="space-y-3">
              <p><strong>Number of Applications:</strong> {selectedWebCount}</p>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {selectedWebVulnerabilities.map(vuln => (
                      <Badge key={vuln} variant="outline" className="text-xs">
                        {vuln === 'sql-injection' ? 'SQL Injection' :
                         vuln === 'xss' ? 'Cross-Site Scripting (XSS)' :
                         vuln === 'csrf' ? 'Cross-Site Request Forgery (CSRF)' :
                         vuln === 'authentication' ? 'Authentication & Session Management' :
                         vuln === 'authorization' ? 'Authorization & Access Control' :
                         vuln === 'file-upload' ? 'File Upload Vulnerabilities' :
                         vuln === 'business-logic' ? 'Business Logic Flaws' :
                         vuln === 'all' ? 'All Common Vulnerabilities' : vuln}
                      </Badge>
                    ))}
                  </div>
                </div>
        ...
        {selectedServices.includes('mobile') && (
          ...
          >
            ...
            <CardContent className="space-y-3">
              <p><strong>Number of Applications:</strong> {selectedMobileCount}</p>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {selectedMobileFeatures.map(feature => (
                      <Badge key={feature} variant="outline" className="text-xs">
                        {feature === 'biometric-auth' ? 'Biometric Authentication' :
                         feature === 'push-notifications' ? 'Push Notifications' :
                         feature === 'location-services' ? 'Location Services' :
                         feature === 'camera' ? 'Camera Access' :
                         feature === 'file-storage' ? 'Local File Storage' :
                         feature === 'api-integration' ? 'API Integration' :
                         feature === 'offline-mode' ? 'Offline Functionality' :
                         feature === 'payment' ? 'Payment Processing' : feature}
                      </Badge>
                    ))}
                  </div>
                </div>
        ...
        {selectedServices.includes('api') && (
          ...
          >
            ...
            <CardContent className="space-y-3">
              <p><strong>Number of Endpoints:</strong> {selectedApiCount}</p>
                    ...
                    {selectedApiTypes.map(type => (
                      <Badge key={type} variant="secondary" className="text-xs">
                        {type === 'rest' ? 'REST APIs' :
                         ...
                         type === 'microservices' ? 'Microservices APIs' : type}
                      </Badge>
                    ))}
                  </div>
                </div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {selectedApiAuthMethods.map(auth => (
                      <Badge key={auth} variant="outline" className="text-xs">
                        {auth === 'api-key' ? 'API Keys' :
                         auth === 'oauth2' ? 'OAuth 2.0' :
                         auth === 'jwt' ? 'JWT Tokens' :
                         auth === 'basic-auth' ? 'Basic Authentication' :
                         auth === 'bearer-token' ? 'Bearer Tokens' :
                         auth === 'certificate' ? 'Client Certificates' :
                         auth === 'custom' ? 'Custom Authentication' :
                         auth === 'none' ? 'No Authentication' : auth}
                      </Badge>
                    ))}
                  </div>
                </div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {selectedApiSensitiveData.map(data => (
                      <Badge key={data} variant="outline" className="text-xs">
                        {data === 'pii' ? 'Personal Identifiable Information (PII)' :
                         data === 'financial' ? 'Financial Data' :
                         data === 'health' ? 'Health Information' :
                         data === 'credentials' ? 'User Credentials' :
                         data === 'business' ? 'Business Data' :
                         data === 'none' ? 'No Sensitive Data' : data}
                      </Badge>
                    ))}
                  </div>
                </div>
        ...
        {selectedServices.includes('network') && (
          ...
          >
            ...
            <CardContent className="space-y-3">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {selectedNetworkComponents.map(component => (
                      <Badge key={component} variant="secondary" className="text-xs">
                        {component === 'network-devices' ? 'Network Devices' :
                         component === 'firewalls' ? 'Firewalls' :
                         component === 'routers' ? 'Routers & Switches' :
                         component === 'wireless' ? 'Wireless Networks' :
                         component === 'vpn' ? 'VPN Gateways' :
                         component === 'load-balancers' ? 'Load Balancers' :
                         component === 'network-monitoring' ? 'Network Monitoring Systems' :
                         component === 'network-security' ? 'Network Security Appliances' : component}
                      </Badge>
                    ))}
                  </div>
                </div>
              ...
              {networkConcernsText && (
                <div>
                  <p className="font-semibold mb-2">Security Concerns:</p>
                    ...
                    {selectedInfrastructureComponents.map(component => (
                      <Badge key={component} variant="secondary" className="text-xs">
                        {component === 'servers' ? 'Servers' :
                         component === 'cloud' ? 'Cloud Infrastructure' :
                         component === 'containers' ? 'Containers/Docker' :
                         component === 'databases' ? 'Database Servers' :
                         component === 'storage' ? 'Storage Systems' :
                         component === 'virtualization' ? 'Virtualization Platforms' :
                         component === 'backup' ? 'Backup Systems' :
                         component === 'monitoring' ? 'Infrastructure Monitoring' : component}
                      </Badge>
                    ))}
                  </div>
                </div>
                    ...
                    {selectedCloudPlatforms.map(platform => (
                      <Badge key={platform} variant="outline" className="text-xs">
                        {platform === 'aws' ? 'Amazon Web Services (AWS)' :
                         ...
                         platform === 'on-premise' ? 'On-Premise Infrastructure' :
                         platform === 'hybrid' ? 'Hybrid Cloud' :
                         platform === 'none' ? 'No Cloud Services' : platform}
                      </Badge>
                    ))}
                  </div>
                </div>
      ...
      <Card className="max-w-2xl mx-auto">
        <CardContent className="p-12 text-center">
          ...
          <p className="text-lg text-muted-foreground mb-8">
            Thank you for completing the needs assessment. Our team will review your requirements and get back to you within 2-3 business days.
          </p>
          <Button asChild>
        ...
        {isEditMode && (
          <div className="bg-orange-50 border border-orange-200 rounded-lg p-4 mb-6">
            ...
            <p className="text-sm text-orange-700 mt-1">
              You are editing your assessment. Click "Update" to save changes and return to the summary.
            </p>
          </div>
        )}
        ...
        <div className="mb-8">
          ...
          <div className="flex items-center justify-between text-sm">
            {STEPS.map(step => {
              ...
              const isServiceSelected = isServiceStep ? selectedServices.includes(step.condition) : true;
              ...
              return (
                <div
                  ...
                  onClick={() => {
                    ...
                    if (currentStep === 8 && (stepState === 'active' || stepState === 'completed')) {
