      ...
      try {
        ...
        if (result.tables.hermes_leads) {
          try {
            ...
            errors.push(`hermes_leads table query failed: ${error instanceof Error ? error.message : 'Unknown error'}`);
          }
        }
...
async function main() {
  ...
  try {
    ...
    console.log(`   Status: ${health.indexes.status === 'optimal' ? '✅' : '⚠️'} ${health.indexes.status}`);
